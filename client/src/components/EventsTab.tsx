          ...
          <div className="flex gap-2 pt-1">
            ...
            {!isEdit && (
              <button type="button" disabled={saving} onClick={e => onSave(e as any, true)}
                className="flex-1 bg-rose hover:bg-[#5AAEAA] disabled:opacity-60 text-white font-medium rounded-xl py-2.5 text-sm transition">
                Опублікувати
              </button>
            )}
          </div>
    ...
    <div className="max-w-2xl">
      <div className="flex justify-end mb-4">
        <button onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 bg-rose hover:bg-[#5AAEAA] text-white font-medium rounded-xl px-5 py-2.5 text-sm transition">
                              ...
                              <div className="flex flex-wrap gap-2 mt-3">
                                {reg.status === 'PENDING' && (
                                  <button onClick={() => handleSendPayment(ev.id, reg.id)} disabled={actionLoading === reg.id + '-pay'}
                                    className="text-xs bg-rose hover:bg-[#5AAEAA] disabled:opacity-50 text-white font-medium rounded-lg px-3 py-1.5 transition">
                                    Надіслати реквізити
                                  </button>
                                )}
