  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto">
        ...
        {/* Filter tabs */}
        <div className="flex gap-1 bg-white/80 rounded-2xl p-1 border border-[#D5E6E5]/60 shadow-[0_1px_8px_rgba(160,120,100,0.06)] overflow-x-auto mb-6">
          {(['upcoming', 'completed', 'all'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={filter === f
                ? 'bg-[#4D8A85] text-white rounded-xl px-3 py-1.5 text-xs font-medium shadow-sm whitespace-nowrap transition'
                : 'text-warm-mid hover:text-warm-dark hover:bg-[#EBF5F3] rounded-xl px-3 py-1.5 text-xs font-medium whitespace-nowrap transition'}
            >
