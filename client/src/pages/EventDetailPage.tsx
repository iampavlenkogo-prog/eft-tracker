...
export default function EventDetailPage() {
  ...
  const IcChip = ({ children }: { children: React.ReactNode }) => (
    <span
      className="w-[42px] h-[42px] rounded-[13px] flex items-center justify-center shrink-0 bg-[#F3DDD1] text-[#F45A34]"
  ...
  const RegChip = ({ children }: { children: React.ReactNode }) => (
    <span
      className="w-[38px] h-[38px] rounded-[12px] flex items-center justify-center shrink-0 bg-[#F3DDD1] text-[#F45A34]"
      style={{ boxShadow: CLAY_SM }}
    >
      {children}
    </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_354px] gap-8 items-start">
          ...
          <div className="min-w-0 order-2 lg:order-1">
            ...
            >
              ...
              <div className="absolute left-5 top-5">
                {isCancelled ? (
                  ...
                  <span className="inline-flex items-center gap-[7px] px-4 py-[9px] rounded-full bg-[rgba(252,248,245,.92)] text-[#F45A34] text-[12px] font-extrabold tracking-[.08em] uppercase" style={{ boxShadow: CLAY_SM }}>
                    ✦ Подія
                  </span>
                )}
              </div>
            ...
            <div className="inline-flex items-center gap-[7px] mt-6 text-[13px] font-bold text-[#A99CA1]">
              <span className="w-2 h-2 rounded-full bg-[#F45A34]" />
            ...
            {isOrganizer && (
              <div className="mt-9 rounded-[28px] p-6" style={{ background: '#FCF8F5', boxShadow: CLAY }}>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 bg-[#F3DDD1] border border-[rgba(244,90,52,.2)] text-[#F45A34] rounded-full text-sm font-bold hover:bg-[#F45A34] hover:text-white transition"
                >
                  <Send size={15} />
                    ...
                    {!pendingReceiptFile ? (
                      <div
                        ...
                        className={`border-2 border-dashed rounded-[18px] p-5 text-center cursor-pointer transition ${
                          dragOver ? 'border-[#F45A34] bg-[#F3DDD1]' : 'border-[#DDD4D0] hover:border-[rgba(244,90,52,.5)] hover:bg-[#FCF8F5]'
                        }`}
                      >
                        <Upload size={22} className="text-[#F45A34]/50 mx-auto mb-2" />
                      ...
                      <div className="border border-[#E4CFC0] rounded-[18px] px-4 py-3 bg-[#FCF8F5] flex items-center gap-3">
                        ...
                        <button onClick={() => setPendingReceiptFile(null)} className="text-[#A99CA1] hover:text-[#F45A34] transition shrink-0">
                          <X size={15} />
                        </button>
                      </div>
                    )}
            ...
            <div className="px-6 pt-6 pb-4 border-b border-[rgba(120,90,95,.10)] shrink-0">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-[#F45A34] font-bold mt-0.5 truncate">{event.title}</p>
                </div>
            ...
            <div className="px-6 pt-6 pb-4 border-b border-[rgba(120,90,95,.10)] shrink-0">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-[12.5px] text-[#F45A34] font-bold mt-0.5 leading-snug">
                  ...
                  {[
                    ...
                    <label key={i} onClick={() => setConsents(prev => prev.map((v, idx) => idx === i ? !v : v))} className="flex items-start gap-3 cursor-pointer group">
                      <div className={`mt-0.5 w-5 h-5 shrink-0 rounded border-2 flex items-center justify-center transition-all ${
                        consents[i] ? 'bg-[#F45A34] border-[#F45A34]' : 'border-[#E4CFC0] group-hover:border-[rgba(244,90,52,.5)]'
                      }`}>
                        {consents[i] && (
            ...
            <div className="px-6 pt-4 pb-5 border-t border-[rgba(120,90,95,.10)] shrink-0 space-y-3">
              <div className="bg-[#FFF4EC] border border-[rgba(176,107,126,.15)] rounded-[18px] px-4 py-3">
                <p className="text-[11.5px] text-[#7A6E73] leading-relaxed">
                  <span className="font-bold text-[#F45A34]">Важливо:</span> якщо під час події демонструються записи сесій або клінічні матеріали, їх перегляд дозволений лише в межах цієї події та не передбачає жодного копіювання, збереження чи подальшого поширення.
                </p>
              </div>
              <button
