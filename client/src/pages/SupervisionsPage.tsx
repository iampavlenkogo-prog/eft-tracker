...
const BOOKING_STATUS: Record<string, { label: string; cls: string }> = {
  ...
  APPROVED:  { label: '♡ Підтверджено',        cls: 'bg-[#DCE7EA] text-[#5E828E]' },
...
const PAYMENT_LABELS: Record<string, { label: string; cls: string }> = {
  ...
  CONFIRMED:        { label: 'Оплату підтверджено', cls: 'bg-[#DCE7EA] text-[#5E828E]' },
          ...
          <div className="flex items-center gap-3 flex-wrap mt-[22px]">
            ...
            {activeTab !== 'bookings' ? (
              <button
                ...
                style={{ background: 'linear-gradient(135deg,#F45A34,#D93818)', boxShadow: '-4px -4px 12px rgba(255,255,255,.4),10px 12px 26px rgba(244,90,52,.40)' }}
              >
                <Plus size={17} />Додати
              </button>
            ) : (
        ...
        <aside className="hidden lg:flex flex-col gap-5 sticky top-[92px]">
          ...
          <div className="rounded-[36px] p-[24px_26px]" style={{ background: 'var(--surface)', boxShadow: 'var(--clay)' }}>
            ...
            <Link to="/slots"
              ...
              style={{ background: 'linear-gradient(135deg,#F45A34,#D93818)', boxShadow: 'var(--clay-sm)' }}>
              Переглянути слоти →
            </Link>
          </div>
